...
// create more work.
//
// The legacy path also tries to minimize activations and deactivations for
...
void G1ConcurrentRefineThreadsNeeded::update(uint active_threads,
  ...
  // threads needed.  On the legacy path the current thread is treated as
  // running and mutator refinement prevents further accumulation.  That might
  // not be sufficient, but hopefully we were already reasonably close.
  if (_predicted_time_until_next_gc_ms <= _update_period_ms) {
