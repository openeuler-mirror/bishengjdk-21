...
class G1PostEvacuateCollectionSetCleanupTask2::ProcessEvacuationFailedRegionsTask : public G1AbstractSubTask {
  ...
  public:
    ...
    bool do_heap_region(HeapRegion* r) override {
      ...
      // Concurrent mark does not mark through retained regions, so preserve
      // marking state only for failed regions that will be marked through.
...
public:
  ...
  virtual ~FreeCollectionSetTask() {
    ...
    bool has_new_retained_regions =
      p->sum_thread_work_items(G1GCPhaseTimes::RestoreRetainedRegions,
                               G1GCPhaseTimes::RestoreRetainedRegionsRetainedNum) != 0;
    if (has_new_retained_regions) {
      _g1h->collection_set()->candidates()->sort_by_efficiency();
    }
#endif /* AARCH64 */
    report_statistics();
    for (uint worker = 0; worker < _active_workers; worker++) {
  ...
  void do_work(uint worker_id) override {
    ...
    cl.report_timing();
#ifdef AARCH64
    _g1h->phase_times()->record_or_add_thread_work_item(
