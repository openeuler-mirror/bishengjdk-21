...
#ifdef AARCH64
class G1RefineRegionClosure : public HeapRegionClosure {
  ...
  void verify_card_pair_refers_to_same_card(CardValue* source_card, CardValue* dest_card) {
    ...
    HeapRegion* refinement_r = g1h->heap_region_containing(g1h->refinement_table()->addr_for(source_card));
    HeapRegion* card_r = g1h->heap_region_containing(g1h->card_table()->addr_for(dest_card));
...
#ifdef AARCH64
bool G1ConcurrentRefineSweepTask::sweep_completed() const {
  return Atomic::load(&_sweep_completed);
}
