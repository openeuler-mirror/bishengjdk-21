...
bool G1CardTableClaimTable::has_unclaimed_cards(uint region) {
  ...
  return Atomic::load(&_card_claims[region]) < HeapRegion::CardsPerRegion;
}
...
uint G1CardTableClaimTable::claim_all_cards(uint region) {
  return claim_cards(region, (uint)HeapRegion::CardsPerRegion);
}
...
bool G1CardTableChunkClaimer::has_next() {
  ...
  return (_cur_claim < HeapRegion::CardsPerRegion);
}
