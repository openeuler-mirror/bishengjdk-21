...
G1CardTableClaimTable::G1CardTableClaimTable(uint chunks_per_region) :
  ...
  _cards_per_chunk(checked_cast<uint>(HeapRegion::CardsPerRegion / chunks_per_region))
{
...
void G1CardTableClaimTable::reset_all_to_claimed() {
  for (uint i = 0; i < _max_reserved_regions; i++) {
    _card_claims[i] = (uint)HeapRegion::CardsPerRegion;
  }
}

void G1CardTableClaimTable::heap_region_iterate_from_worker_offset(HeapRegionClosure* cl, uint worker_id, uint max_workers) {
  ...
  for (uint count = 0; count < n_regions; count++) {
    ...
    HeapRegion* r = G1CollectedHeap::heap()->region_at(index);
