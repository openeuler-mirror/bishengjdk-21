...
bool ProfileCacheMethodHold::is_alive() const {
  if (_resolved_method == nullptr || _resolved_method->constMethod() == nullptr || _resolved_method->constants() == nullptr || _resolved_method->constants()->pool_holder() == nullptr) {
    return false;
  }
...
ProfileCacheClassHolder::ProfileCacheClassHolder(Symbol* name, Symbol* loader_name,
  ...
  : _class_name(name),
    ...
    _class_method_list(new (mtInternal)
