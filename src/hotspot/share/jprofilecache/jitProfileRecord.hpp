...
public:
    ...
    ~ClassSymbolEntry() {
        if (_class_name != nullptr) _class_name->decrement_refcount();
        if (_class_loader_name != nullptr) _class_loader_name->decrement_refcount();
        if (_class_path != nullptr) _class_path->decrement_refcount();
    }
...
private:
  ...
  void write_data_layout(DataLayout* value);
