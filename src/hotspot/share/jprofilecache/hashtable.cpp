...
#include "jprofilecache/hashtable.hpp"
...
template <MEMFLAGS F> BasicHashtableEntry<F>* BasicHashtable<F>::new_entry_free_list() {
  BasicHashtableEntry<F>* entry = nullptr;
...
template <MEMFLAGS F> BasicHashtableEntry<F>* BasicHashtable<F>::new_entry(unsigned int hashValue) {
  ...
  if (entry == nullptr) {
    if (_first_free_entry + _entry_size >= _end_block) {
      ...
      len = 1 << log2i(len); // round down to power of 2
...
template <class T, MEMFLAGS F> void RehashableHashtable<T, F>::move_to(RehashableHashtable<T, F>* new_table) {
  ...
  for (int i = 0; i < new_table->table_size(); ++i) {
    for (HashtableEntry<T, F>* p = this->bucket(i); p != nullptr; ) {
...
template <MEMFLAGS F> void BasicHashtable<F>::free_buckets() {
  if (nullptr != _buckets) {
    ...
    if (!UseSharedSpaces ||
        !MetaspaceShared::is_in_shared_metaspace(_buckets)) {
...
template <MEMFLAGS F> void BasicHashtable<F>::bulk_free_entries(BucketUnlinkContext* context) {
  if (context->_num_removed == 0) {
    assert(context->_removed_head == nullptr && context->_removed_tail == nullptr,
  ...
  while (true) {
    ...
    BasicHashtableEntry<F>* old = Atomic::cmpxchg(&_free_list, context->_removed_head, current);
    if (old == current) {
      break;
    }
    current = old;
  }
  Atomic::add(&_number_of_entries, context->_num_removed);
}
...
template <class T, MEMFLAGS F> int RehashableHashtable<T, F>::literal_size(oop oop) {
  ...
  assert(oop != nullptr && oop->klass() == vmClasses::String_klass(), "only strings are supported");
...
template <MEMFLAGS F> void BasicHashtable<F>::verify() {
  ...
  for (int i = 0; i < table_size(); i++) {
    for (BasicHashtableEntry<F>* p = bucket(i); p != nullptr; p = p->next()) {
      ++count;
    }
  }
