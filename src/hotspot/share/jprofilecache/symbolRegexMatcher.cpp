...
#include "jprofilecache/symbolRegexMatcher.hpp"
...
template <MEMFLAGS F> SymbolRegexMatcher<F>::SymbolRegexMatcher(const char* regexes)
  : _patterns(new (F) GrowableArray<SymbolPatternMatcher>(SYMBOLREGEXMATCHER_INIT_SIZE, F)) {
  assert(regexes != nullptr, "illegal regexes");
