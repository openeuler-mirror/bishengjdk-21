...
const char* JitProfileCacheFileParser::read_string() {
  ...
  if (actual_string_length == 0) {
    log_warning(jprofilecache)("[JitProfileCache] WARNING : Parsed empty symbol at position %d\n", _position);
    return "";
  } else if (actual_string_length > max_symbol_length()) {
    log_error(jprofilecache)("[JitProfileCache] ERROR : The parsed symbol length exceeds %d\n", max_symbol_length());
    return nullptr;
  } else {
...
bool JitProfileCacheFileParser::logparse_illegal_check(const char* s, bool ret_value, int end_position) {
  if (_position > end_position) {
    log_error(jprofilecache)("[JitProfileCache] ERROR : read out of bound, "
                      "file format error");
    return ret_value;
  }
  if (s == nullptr) {
    _position = end_position;
    log_error(jprofilecache)("[JitProfileCache] ERROR : illegal string in log file");
    return ret_value;
  }
  return true;
}
...
bool JitProfileCacheFileParser::logparse_illegal_count_check(int cnt, bool ret_value, int end_position) {
  ...
  if ((u4)cnt > MAX_COUNT_VALUE) {
    _position = end_position;
    log_error(jprofilecache)("[JitProfileCache] ERROR : illegal count ("
...
bool JitProfileCacheFileParser::should_ignore_this_class(Symbol* symbol) {
  ...
  if (::strstr(name, CGLIB_SIG) != nullptr ||
      ::strstr(name, ACCESSER_SUFFIX) != nullptr ||
      (UseJProfilingCacheSystemBlackList &&
       JitProfileCacheUtils::is_in_unpreloadable_classes_black_list(name))) {
    log_debug(jprofilecache)("[JitProfileCache] DEBUG : Ignore class %s", name);
    return true;
  }
...
bool JitProfileCacheFileParser::parse_header() {
  ...
  if (parse_magic_number != JPROFILECACHE_MAGIC_NUMBER
    ...
    _is_valid = false;
    log_error(jprofilecache)("[JitProfileCache] ERROR : illegal header");
    return false;
  }
  // valid appid
  if (CompilationProfileCacheAppID != 0 && CompilationProfileCacheAppID != appid) {
    _is_valid = false;
    log_error(jprofilecache)("[JitProfileCache] ERROR : illegal CompilationProfileCacheAppID");
    return false;
  }
  // valid crc32
  ...
  if (parse_crc32_recorded != crc32_actual) {
    _is_valid = false;
    log_error(jprofilecache)("[JitProfileCache] ERROR : JitProfile crc32 check failure");
    return false;
  }
...
ProfileCacheMethodHold* JitProfileCacheFileParser::parse_method() {
  ...
  if (compilation_type != 0 && compilation_type != 1) {
    log_error(jprofilecache)("[JitProfileCache] ERROR : illegal compilation type in JitProfile");
    _position = end_position;
    return nullptr;
  }
  // parse method info
  ...
  if (entry == nullptr) {
    log_info(jprofilecache)("[JitProfileCache] WARNING : class %s is missed in method parse", parse_class_name_char);
    _position = end_position;
    return nullptr;
  }
