...
ProfileCacheMethodHold* MethodHolderIterator::next() {
  ...
  while (_holder_index > 0) {
    ...
    if (entry->method_holder() != nullptr) {
...
bool ProfileCacheClassChain::ProfileCacheClassChainEntry::is_all_initialized() {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (k != nullptr && k->is_not_initialized() && !k->is_in_error_state() ) {
      return false;
    }
  }
  return true;
...
bool ProfileCacheClassChain::ProfileCacheClassChainEntry::contains_redefined_class() {
  ...
  for (int i = 0; i < len; i++) {
    InstanceKlass* k = resolved_klasses()->at(i);
    if (k != nullptr && k->has_been_redefined()) {
      ResourceMark rm;
      log_warning(jprofilecache)("[JitProfileCache] WARNING: ignore redefined class after API"
...
bool ProfileCacheClassChain::try_transition_to_state(ClassChainState new_state) {
  ...
  if (old_state == new_state) {
    log_warning(jprofilecache)("JProfileCache [WARNING]: profilecache state has already been %s Doesn't need transferred to %s",
  ...
  if (can_transfer) {
    if (Atomic::cmpxchg((jint*)&_state, (jint)old_state, (jint)new_state) == old_state) {
      return true;
    } else {
      log_warning(jprofilecache)("JProfileCache [WARNING]: failed to transfer profilecache state from %s to %s, conflict with other operation",
                                              get_state(old_state), get_state(new_state));
      return false;
    }
  } else {
    log_warning(jprofilecache)("JProfileCache [WARNING]: can not transfer profilecache state from %s to %s",
                                            get_state(old_state), get_state(new_state));
    return false;
  }
}
...
void ProfileCacheClassChain::mark_loaded_class(InstanceKlass* k, ProfileCacheClassEntry* class_entry) {
  ...
  if (holder != nullptr) {
    ...
    ResourceMark rm;
    log_debug(jprofilecache)("[JitProfileCache] DEBUG : class %s is not in profile",
                      k->name()->as_C_string());
  }
...
void ProfileCacheClassChain::handle_duplicate_class(InstanceKlass *k, int chain_index) {
  ...
  if (!t->is_super_class_resolution_active()) {
    ...
    ResourceMark rm;
    log_warning(jprofilecache)("[JitProfileCache] WARNING : Duplicate load class %s at index %d",
...
void ProfileCacheClassChain::resolve_class_methods(InstanceKlass* k, ProfileCacheClassHolder* holder, int chain_index) {
  ...
  {
    ResourceMark rm;
    log_debug(jprofilecache)("[JitProfileCache] DEBUG : class %s at index %d method_list has been recorded",
                      k->name()->as_C_string(), chain_index);
  }
  holder->set_resolved();
}
...
void ProfileCacheClassChain::compile_methodholders_queue(Stack<ProfileCacheMethodHold*, mtInternal>& compile_queue) {
  while (!compile_queue.is_empty()) {
    ...
    if (HAS_PENDING_EXCEPTION) {
      ResourceMark rm;
      log_warning(jprofilecache)("[JitProfileCache] WARNING: Exceptions happened in compiling %s",
...
void ProfileCacheClassChain::precompilation() {
  ...
  for ( int index = 0; index < length(); index++ ) {
    ...
    if (klass != nullptr) {
      ...
      klass->initialize((JavaThread *)THREAD);
      if (HAS_PENDING_EXCEPTION) {
        Symbol *loader = JitProfileCacheUtils::get_class_loader_name(klass->class_loader_data());
        ResourceMark rm;
        log_error(jprofilecache)("[JitProfileCache] ERROR: Exceptions happened in initializing %s being loaded by %s",
...
void ProfileCacheClassChain::unload_class() {
  ...
  for (int i = 0; i < length(); i++) {
    ...
    for (int i = 0; i < array->length(); i++) {
      ...
      // if data is nullptr or not alive should be remove
      if (data == nullptr || !data->is_alive()) {
    ...
    for (ProfileCacheMethodHold* holder = entry->method_holder(); holder != nullptr;
         holder = holder->next()) {
      // if method not compile or deopted continue
      if (holder->resolved_method() == nullptr) {
        continue;
      }
      if (!holder->is_alive()) {
        // process the method in the class.
        holder->set_resolved_method(nullptr);
      }
    }
  }
}
...
ProfileCacheMethodHold* ProfileCacheClassChain::resolve_method_info(Method* method, ProfileCacheClassHolder* holder) {
  ...
  if (mh == nullptr) {
    return mh;
  } else if (mh->resolved_method() == nullptr) {
    mh->set_resolved_method(method);
    method->set_jpc_method_holder(mh);
    return mh;
  } else {
    log_info(jprofilecache)("[JitProfileCache] method %s is resolved again",
...
void ProfileCacheClassChain::preload_class_in_constantpool() {
  ...
  int klass_index = 0;
  while (true) {
    InstanceKlass* current_k = nullptr;
    {
      MutexLocker mu(ProfileCacheClassChain_lock);
      if (index == length()) {
        break;
      ...
      assert(array != nullptr, "should not be nullptr");
