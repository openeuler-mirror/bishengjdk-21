...
JavaThread* JitProfileCacheThread::_jprofilecache_thread = nullptr;
...
void JitProfileCacheThread::run(TRAPS) {
  ...
  }
  {
    ...
    if (_thread == nullptr || _thread->osthread() == nullptr) {
    ...
    _thread->set_threadOopHandles(thread_oop());
    Threads::add(_thread);
...
void JitProfileCacheThread::print_jit_profile_cache_thread_info_on(outputStream* st) {
  MutexLocker mu(JitProfileCachePrint_lock);
  if (_jprofilecache_thread == nullptr || !is_active()) {
      return;
  }
