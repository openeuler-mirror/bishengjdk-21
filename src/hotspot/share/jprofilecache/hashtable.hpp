...
#ifndef SHARE_JPROFILECACHE_HASHTABLE_HPP
#define SHARE_JPROFILECACHE_HASHTABLE_HPP
...
protected:
  ...
  void copy_freelist(BasicHashtable* src) {
    ...
    src->_free_list = nullptr;
    _first_free_entry = src->_first_free_entry;
    src->_first_free_entry = nullptr;
    _end_block = src->_end_block;
    src->_end_block = nullptr;
  }
  ...
  struct BucketUnlinkContext {
    ...
    BucketUnlinkContext() : _num_processed(0), _num_removed(0), _removed_head(nullptr), _removed_tail(nullptr) {
    }
...
public:
  unsigned int compute_hash(Symbol* name, ClassLoaderData* loader_data) {
    ...
    // loader is nullptr with CDS
    assert(loader_data != nullptr || UseSharedSpaces || DumpSharedSpaces,
           "only allowed with shared spaces");
    unsigned int loader_hash = loader_data == nullptr ? 0 : loader_data->identity_hash();
...
public:
  ...
  V* lookup(K key) const {
    ...
    for (KVHashtableEntry* e = bucket(index); e != nullptr; e = e->next()) {
...
#endif // SHARE_JPROFILECACHE_HASHTABLE_HPP
