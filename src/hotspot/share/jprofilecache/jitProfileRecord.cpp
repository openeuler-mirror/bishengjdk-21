...
const char* JitProfileRecorder::_auto_jpcfile_name = nullptr;
const char* JitProfileRecorder::_auto_temp_jpcfile_name = nullptr;
FILE*       JitProfileRecorder::_auto_jpcfile_filepointer = nullptr;
...
void JitProfileRecorder::set_logfile_name(const char* name)  {
  _record_file_name = make_log_name(name, nullptr);
}
...
const char* JitProfileRecorder::auto_temp_jpcfile_name() {
  if (_auto_temp_jpcfile_name == nullptr) {
...
void JitProfileRecorder::init() {
  ...
  if (JProfilingCacheCompileAdvance) {
    log_error(jprofilecache)("[JitProfileCache] ERROR: JProfilingCacheCompileAdvance and JProfilingCacheRecording cannot be enabled at the same time");
    _recorder_state = IS_ERR;
    return;
  }
  if (!ProfileInterpreter) {
    log_error(jprofilecache)("[JitProfileCache] ERROR: ProfileInterpreter must be enable");
    _recorder_state = IS_ERR;
    return;
  }
  // disable class unloading
  if (ClassUnloading) {
    log_error(jprofilecache)("[JitProfileCache] ERROR: ClassUnloading must be disabled");
    _recorder_state = IS_ERR;
    return;
  }

  if (UseG1GC && ClassUnloadingWithConcurrentMark) {
    log_error(jprofilecache)("[JitProfileCache] ERROR: if use G1 gc, ClassUnloadingWithConcurrentMark must be disabled");
    _recorder_state = IS_ERR;
    return;
  }
  ...
  } else if (ProfilingCacheFile == nullptr) {
  ...
  log_debug(jprofilecache)("[JitProfileCache] begin to collect, log file is %s", logfile_name());
}
...
int JitProfileRecorder::assign_class_init_order(InstanceKlass* klass) {
  ...
  if (record_name == nullptr || record_name->utf8_length() == 0) {
    return -1;
  }
...
JitProfileRecorderEntry* JitProfileRecordDictionary::add_method(unsigned int method_hash, Method* method, int bci) {
  ...
  log_debug(jprofilecache)("[JitProfileCache] Record method %s", method->name_and_sig_as_C_string());
...
JitProfileRecorderEntry* JitProfileRecordDictionary::find_entry(unsigned int hash, Method* method) {
  ...
  for (JitProfileRecorderEntry* p = bucket(index); p != nullptr; p = p->next()) {
    if (p->literal() == method) {
      return p;
    }
  }
...
void JitProfileRecorder::write_data_layout(DataLayout* value) {
  int size = value->size_in_bytes();
  write_u4(size);
  _profilelog->write((char*)value, size);
  _pos += size;
}
...
void JitProfileRecorder::write_inited_class() {
  ...
  while (node != nullptr) {
    ...
    const char* record_class_loader_name = nullptr;
    if (record_entry->class_loader_name() == nullptr) {
      record_class_loader_name = "nullptr";
    } else {
      record_class_loader_name = record_entry->class_loader_name()->as_C_string();
    }
    const char* path = nullptr;
    if (record_entry->path() == nullptr) {
...
void JitProfileRecorder::write_profilecache_record(Method* method, int bci, int order) {
  ...
  if (method_counters != nullptr) {
    ...
    log_warning(jprofilecache)("[JitProfileCache] WARNING: the method counter is nullptr");
    write_u4((u4)0);
    write_u4((u4)0);
    write_u4((u4)0);
    write_u4((u4)0);
...
void JitProfileRecorder::write_method_profiledata(MethodData* mdo) {
  ...
  if (arg_info != nullptr) { count++; }
  write_u4((u4)count);
  if (arg_info != nullptr) {
    write_data_layout((DataLayout*)(arg_info->dp()));
    log_info(jprofilecache)("Record ArgInfoData of method: %s",
  ...
  for (dp_src = mdo->first_data(); mdo->is_valid(dp_src) ;dp_src = mdo->next_data(dp_src)) {
    if (is_recordable_data(dp_src)) {
      ...
      write_data_layout(dp);
      log_info(jprofilecache)("Record ProfileData(Tag:%d) on bytecode(%d) of method: %s",
...
void JitProfileRecorder::record_class_info(InstanceKlass* klass) {
  ...
  if (record_class_loader != nullptr) {
...
bool JitProfileRecorder::flush_record() {
  ...
  if (JProfilingCacheAutoArchiveDir != nullptr) {
    ...
    _profilelog = new (mtInternal) randomAccessFileStream(logfile_name(), "wb+");
  }
